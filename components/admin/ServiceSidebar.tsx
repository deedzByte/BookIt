      ...
      <div className="mb-4 flex items-center justify-between gap-3 border-b border-border/70 pb-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">Catalog</p>
            <h2 className="truncate text-sm font-semibold text-foreground">Services <span className="text-cyan-700 text-[12px]">({services.length})</span></h2>
          </div>
        </div>
            <Button
              variant="ghost"
      ...
      <div className="mb-3 space-y-3">
        <div className="relative">
          ...
          <Input
            ...
            className="h-9 pl-9 pr-9 rounded-xl bg-muted/30 border-border/60 focus-visible:ring-primary/20"
          />
          {searchQuery && (
            <button
      ...
      <ScrollArea className="flex-1 pr-1">
        <div className="space-y-2">
          {filteredServices.length === 0 ? (
            ...
            <AddServiceDialog
              open={showAddService}
              onOpenChange={setShowAddService} onAddService={function (service: { name: string; isActive: boolean; category: any[]; }): void {
